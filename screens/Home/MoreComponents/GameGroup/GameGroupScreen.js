...
const GameGroupThumbnail = props => {
    ...
    >
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <View style={{ width: '30%' }}>
                <Text style={{ color: 'grey' }}>{props.thread.lastpostdate.split("+")[0]}</Text>
...
const GameGroupScreen = ({ navigation, route }) => {
    ...
    async function fetchLocalGameGroups(country) {
        ...
        const url = "https://boardgamegeek.com/xmlapi2/forum?id=" + country + "&page=" + pageNumToRender
    ...
    function filterThreads(threads) {
        if (filterString !== "") {
            let filteredThreads = threads.filter(item => item['$'].subject.includes(filterString));
    ...
    return (
        <View>
                    ...
                    {searchOpen ?
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <SearchBar
                                ...
                                value={filterString}
                                containerStyle={{ width: '100%', backgroundColor: styleconstants.bggpurple }}
                            ...
                            <ScrollView automaticallyAdjustContentInsets={true} ref={scrollRef}>
                                ...
                                <View>
                                    ...
                                    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-evenly', marginTop: 20, alignItems: 'center' }}>
                                        ...
                                        <Icon
                                            name="caretleft"
                                            color={pageNumToRender > 1 ? styleconstants.bggorange : 'lightgrey'}
                                            type="antdesign"
                                            containerStyle={{ margin: 4 }}
                                            size={20}
                                            onPress={() => {
                                        ...
                                        <Text>{(pageNumToRender).toString() + "/" + maxPageNumToRender}</Text>
            ...
            <View style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: 70, backgroundColor: styleconstants.bggpurple }}>

            </View>
            <View style={{ padding: 5, position: 'absolute', top: 0, left: 0, justifyContent: 'center', alignItems: 'center', width: '100%' }}>
